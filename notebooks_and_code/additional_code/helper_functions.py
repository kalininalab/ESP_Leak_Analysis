...
def data_report(df, display_limit=None):
    ...
    def count_unique(x):
        try:
            ...
            if any(isinstance(i, (list, np.ndarray)) for i in x if i is not None):
                return len(x)
            # Handle other cases
            return len(pd.Series(x).dropna().unique())
        except (TypeError, AttributeError):
            # Fallback for cases where the above fails
            return len(x)
...
def plot_roc_curve(fpr, tpr, roc_auc, label, linestyle='solid', color='blue'):
    plt.plot(fpr, tpr, color=color, label=f'{label} , AUC = {roc_auc:.3f}', linewidth=2.0, linestyle=linestyle)
