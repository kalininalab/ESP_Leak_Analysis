...
class SMILESProteinDataset(Dataset):
    def __init__(self,
        ...
        self.prot_dicts = os.listdir(join(embed_dir))
        self.smiles_dicts = os.listdir(join(embed_dir))
    ...
    def _load_smiles_repr(self, smiles_repr_file):
        ...
        return torch.load(smiles_repr_file, map_location=map_loc)
    ...
    def update_subset(self):
        ...
        self.protein_repr = self._load_protein_repr(join(self.embed_dir, self.prot_dicts[self.protein_subset_no]))

        if self.protein_subset_no == 0:
            smiles_repr_file = join(self.embed_dir, self.smiles_dicts[self.smiles_subset_no])
        ...
        help_df = self.df.loc[self.df["molecule ID"].isin(all_subset_smiles)].copy()
        help_df["index"] = list(help_df.index)
        help_df["Uniprot ID"] = [seq[:1018] for seq in help_df["Uniprot ID"]]
        help_df = help_df.loc[help_df["Uniprot ID"].isin(all_subset_sequences)]
    ...
    def __getitem__(self, idx):
        ...
        label, protein, smiles, index = float(self.mappings["Binding"][idx]), self.mappings["Uniprot ID"][idx], \
        self.mappings["molecule ID"][idx], int(self.mappings["index"][idx])
