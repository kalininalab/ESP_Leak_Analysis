...
logging.basicConfig(filename='Hyperparameter_optimization_ESM1bts_and_ECFP_C2.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
train_indices = list(np.load(join(CURRENT_DIR, ".." ,"data","splits", "CV_train_indices_Rand_NOATP.npy"),  allow_pickle=True))
test_indices = list(np.load(join(CURRENT_DIR, ".." ,"data","splits", "CV_test_indices_Rand_NOATP.npy"),  allow_pickle=True))
