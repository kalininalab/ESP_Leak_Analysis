...
def main(args):
    ...
    if len(split_number) not in [2, 3]:
    ...
    log_file = os.path.join(CURRENT_DIR, "..", "data", "Reports",
                            f'Report_{len(split_number)}Splits_{split_method}{Data_suffix}.log')
    ...
    train_output_file = os.path.join(CURRENT_DIR, "..", "data", f"{len(split_number)}splits",
                                     f"train_{split_method}{Data_suffix}_{len(split_number)}S.pkl")
    test_output_file = os.path.join(CURRENT_DIR, "..", "data", f"{len(split_number)}splits",
                                    f"test_{split_method}{Data_suffix}_{len(split_number)}S.pkl")
    val_output_file = os.path.join(CURRENT_DIR, "..", "data", f"{len(split_number)}splits",
                                   f"val_{split_method}{Data_suffix}_{len(split_number)}S.pkl")
    ...
    if split_method in ["C1e", "I1e"]:
        ...
        e_splits, _, _ = datasail_wrapper(split_method, data, split_number)
        for key in e_splits.keys():
            data['split_mol'] = data['ids'].map(e_splits[key][0])
        data_filtered = data[
            (data['split_mol'] == "train") | (data['split_mol'] == "test") | (data['split_mol'] == "val")]
        ...
        _, f_splits, _ = datasail_wrapper(split_method, data_ESP, split_number)
        for key in f_splits.keys():
            data_ESP['split_seq'] = data_ESP['ids'].map(f_splits[key][0])
        data_ESP_filtered = data_ESP[
            (data_ESP['split_seq'] == "train") | (data_ESP['split_seq'] == "test") | (data_ESP['split_seq'] == "val")]
        ...
        logging.info("Start running dataSAIL")
        data = pd.read_pickle(data_file)
        _, _, inter_sp = datasail_wrapper(split_method, data, split_number)
        for key in inter_sp.keys():
            inter_dict = {k[0]: v for k, v in inter_sp[key][0].items()}
            data['split_inter'] = data['ids'].map(inter_dict)
        data_filtered = data[
            (data['split_inter'] == "train") | (data['split_inter'] == "test") | (data['split_inter'] == "val")]
        data_filtered.reset_index(drop=True, inplace=True)
    ...
    if len(split_number) == 2:
        ...
        logging.info(
            f"Report after splitting data by {split_method} split method and check for nan or null cells '\n'{result.to_string()}")
        ...
        logging.info(
            f"Data Report after splitting data by {split_method} split method and check nan or null cells '\n'{result.to_string()}")
        logging.info(f"Total number of samples: {total_samples}")
        logging.info(f"Ratio of test set to dataset: {test_ratio}")
    ...
    train['Sequence'] = train.groupby('Uniprot ID')['Sequence'].transform(
        lambda x: x.fillna(method='ffill').fillna(method='bfill'))
    train['GNN rep (pretrained)'] = train.groupby('molecule ID')['GNN rep (pretrained)'].transform(
        lambda x: x.fillna(method='ffill').fillna(method='bfill'))
    ...
    test['Sequence'] = test.groupby('Uniprot ID')['Sequence'].transform(
        lambda x: x.fillna(method='ffill').fillna(method='bfill'))
    test['GNN rep (pretrained)'] = test.groupby('molecule ID')['GNN rep (pretrained)'].transform(
        lambda x: x.fillna(method='ffill').fillna(method='bfill'))
    ...
    if len(split_number) == 3:
        ...
        val['Sequence'] = val.groupby('Uniprot ID')['Sequence'].transform(
            lambda x: x.fillna(method='ffill').fillna(method='bfill'))
        val['GNN rep (pretrained)'] = val.groupby('molecule ID')['GNN rep (pretrained)'].transform(
            lambda x: x.fillna(method='ffill').fillna(method='bfill'))
    ...
    if len(split_number) == 2:
        result, total_samples, test_ratio = two_split_report(train, test)
        logging.info(f"Report after adding negative data and check for nan or null data cells '\n'{result.to_string()}")
        logging.info(f"Total number of samples: {total_samples}")
    ...
    logging.info(
        Fore.GREEN + f"***** PROCESS COMPLETED: For an overview, "
                     f"please review the Report_{len(split_number)}Splits_{split_method}{Data_suffix}.log file in Reports folder. *****" + Style.RESET_ALL)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--split-number', type=int, nargs='+', required=True,
